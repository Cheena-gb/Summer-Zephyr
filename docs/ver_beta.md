...
## パーツ類の確認
販路やイベントにより内容物とパーツセットの中身は異なります。

- PCB  
  使用時に上を向く面を「オモテ面」、使用時に下を向く面を「ウラ面」と呼称します。
  ビルドガイド内では、切り欠きのある辺を下側に置いている前提で解説します。
- スイッチプレート・バックパネル  
  ABS製です。多少の反り・焦げがある場合がありますが、基本的には問題ありません。  
  反りはねじ止めとスイッチの取付けにより治まります。焦げが気になる場合は軽く磨いてください。
- ネジ  
  M2×12 10本
- スペーサー  
  M2、3.5mmおよび2mm 10本ずつ
- ナット  
  M2 10個
- ピンヘッダ  
  治具用です。短いものが2本ほどあれば十分です。
- ゴム足  
  最低4個、お勧めは6必要です。
...
- プラスドライバー  
  #0
- ピンセット  
  ダイオードのはんだ付け時に使用するほか、導通確認に使用します。非導電性のものを使用の場合はジャンパー線などを別に用意してください。
...
- その他ファンクションキー3個  
  ...
  <details><summary>キーキャップセットで全部揃えられるものはある？</summary>日本語配列用キーキャップセット「Acid Caps」を使用するとすべて間違いなく揃えることができます(とはいえ、上位モデル以外はSpaceの左右幅や形状を揃えることができず、2uコンベックス、2u Backspace、1.75u Shiftなどになるはずです)。これ以外に、明確にすべて揃えることができるセットは把握していません。</details>
...
一旦ここでVialを起動し、「Cheena summer_zephyr」のキーマップが表示されたことを確認してください。  
...
Matrix Testerを起動し、スイッチの入るピン穴にピンセットを突っ込んで導通を見ます。ソケット部はソケットのパッドに押し当てるか[^2]、金属端子部に接触させると導通します。
[^2]:はんだづけした部分ははんだのヤニが皮膜を形成しているため、はんだが付着していない部分がおすすめです
...
一旦はんだづけしてから浮きを修正するのは大変なので、スイッチプレートに噛んでいることを確認する→片足だけはんだづけ→スイッチを基板方向に抑えながらはんだをもう一度溶かして押し込む→もう片方の足をはんだづけ、という手順がお勧めです。
